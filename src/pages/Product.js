...
const Product = () => {
   ...
   const handleCart = () => dispatch(addToCart(product.id));
   ...
   useEffect(
      () => {
         if(productsStore.length === 0){
            ...
            setIsLoading(false)
             // @ts-ignore
            const productsFilter = productsStore.filter(product => product.id === params.id);
   ...
   const descriptionText = product.description ? product.description.slice(3, -4) : null;
   ...
   return (
      <div className="product">
         ...
         <div className="product__info">
            ...
            <button className="button" onClick={handleCart}>Dodaj do koszyka</button>
         </div>
      </div>
   );
};
